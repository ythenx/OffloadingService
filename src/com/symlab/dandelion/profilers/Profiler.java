...
public class Profiler {
	...
	private static final String logFileName = 
			Environment.getExternalStorageDirectory().getPath() + "/PowerDroid-performanceLog.txt";
...
//	private final int MAX_FREQ = 528000; // The maximum frequency for HTC Hero CPU
	...
	public Profiler(Context context, ProgramProfiler progProfiler, NetworkProfiler netProfiler,
	...
	public void updateDB()
	{
        ...
        //{
        	...
        	//selectionArgs = new String[] {lastLogRecord.methodName, lastLogRecord.execLocation, lastLogRecord.networkType, lastLogRecord.networkSubtype};
       //}
        //else
        //{
	...
	public void estimateEnergyConsumption()
	{
		...
		final double divisor = 1000.0 / Constants.PROFILER_UPDATE_PERIOD;
		Log.d("PowerDroid-Energy", "duration: " + duration + "****************");
		estimatedCpuEnergy = estimateCpuEnergy(duration) / divisor;
		Log.d("PowerDroid-Energy", "CPU energy: " + estimatedCpuEnergy + " mJ");
		
		estimatedScreenEnergy = estimateScreenEnergy(duration) / divisor;
		Log.d("PowerDroid-Energy", "Screen energy: " + estimatedScreenEnergy + " mJ");
		
		if(lastLogRecord.execLocation.equals("REMOTE"))
		{
			estimatedWiFiEnergy = estimateWiFiEnergy(duration) / divisor;
...
/*		else if(lastLogRecord.execLocation.equals("REMOTE") && lastLogRecord.networkType.equals("MOBILE"))
	...
	private double estimateCpuEnergy(int duration)
	{
		...
		for(int i = 0; i < duration; i++)
		{
			...
			//Log.d("PowerDroid-Energy", "util freqH freqL cpuON power: " + util + "  " + freqH + "  " + freqL + "  " + cpuON + "  " + estimatedCpuEnergy + "mJ");
